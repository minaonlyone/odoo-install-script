#!/bin/bash
################################################################################
# Script for installing Odoo 20 on Debian 13 (trixie) and newer
# Authors: Yenthe Van Ginneken, Cesar Cordero Rodriguez
# Maintainers: Yenthe Van Ginneken, Cesar Cordero Rodriguez
#-------------------------------------------------------------------------------
...
#   * Python  >= 3.12 (was 3.10 in Odoo 19) -> Debian 13 (trixie) or newer is required,
#     Debian 12 (bookworm) ships Python 3.11 and can no longer run Odoo 20
...
#   "pip" -> pip install -r requirements.txt (needs --break-system-packages on Debian 12+)
...
# Helper: pip install with optional --break-system-packages (Debian 12+ / PEP 668)
pip_install() {
...
detect_arch() {
  ...
  DISTRO_RELEASE="${DISTRO_RELEASE:-13}"
}
...
check_python_version() {
  ...
  if ! python3 -c "import sys; sys.exit(0 if sys.version_info >= (${MIN_PYTHON_MAJOR}, ${MIN_PYTHON_MINOR}) else 1)"; then
    ...
    python3 --version
    printf "Debian 12 (bookworm) ships Python 3.11 and can no longer run Odoo 20.\nUse Debian 13 (trixie) or newer.\n"
    echo "-----------------------------------------------------------"
    exit 1
  fi
...
#--------------------------------------------------
# Preflight: this script drives everything through sudo
#--------------------------------------------------
# A minimal Debian install does not ship sudo, and it cannot be installed from here
# without root in the first place. Fail with an actionable message instead of
# producing hundreds of "sudo: command not found" lines.
if ! command -v sudo >/dev/null 2>&1; then
  echo "------------------------ERROR------------------------------"
  echo "This script needs sudo, which is not installed on this system."
  echo "Install it as root first:"
  echo "    apt-get update && apt-get install -y sudo"
  echo "    usermod -aG sudo <your-user>"
  echo "-----------------------------------------------------------"
  exit 1
fi
...
if [ "$INSTALL_POSTGRESQL_PGDG" = "True" ]; then
    ...
    echo -e "\n---- Installing the default postgreSQL version based on the Debian version ----"
...
# that is patched against Qt, which is NOT the build shipped by Debian.
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $ADMIN_EMAIL != "odoo@example.com" ]  && [ $WEBSITE_NAME != "_" ];then
  sudo apt-get update -y
  # Debian ships certbot and the nginx plugin, no snapd needed.
